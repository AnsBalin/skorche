...
class PipelineManager:
    ...
    def chain(self, task_list: List[Task], queue_in: Queue, queue_out:Queue = None ) -> Queue:
    ...
    def split(self, predicate_fn: Callable, queue_in: Queue, predicate_values: Tuple = (True, False)) -> Tuple[Queue]:
        ...
        out_queue_map = {value: Queue(name=str(value), id=self.new_qid()) for value in predicate_values}
        op = SplitOp(predicate_fn, queue_in, out_queue_map)
        self.ops.append(op)
        self.op_table[op] = {'queues_in': [queue_in], 'queues_out': list(out_queue_map.values())}
    ...
    def batch(self, queue_in: Queue, queue_out: Queue = None, batch_size: int = 1, fill_batch: bool = False):
        if queue_out == None:
            queue_out = Queue(id=self.new_qid())
    ...
    def render_pipeline(self, filename='pipeline', root=None, skip_anon_ques=True) -> None:
        ...
        while len(q):
            ...
            for child in node.children:
                ...
                if skip_anon_ques and child.type == NodeType.QUEUE and len(child.children):
                    child = list(child.children)[0]

                if child.type == NodeType.OP:
                    attr = {'shape': 'rectangle', 'style':'filled', 'color': 'lightgrey'}
