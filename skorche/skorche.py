...
def split(
    predicate_fn: Callable, queue_in: Queue, predicate_values: Tuple = (True, False)
) -> Tuple[Queue]:
    """
...
def batch(
    ...
    queue_out = _global_pipeline.batch(
        queue_in, queue_out=queue_out, batch_size=batch_size, fill_batch=fill_batch
    )
    return queue_out
